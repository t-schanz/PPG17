module run
	...
	subroutine calculate(matrix, mpi_rank, mpi_master, mpi_last, mpi_ierr)
		...
		integer :: request1=1, request2=2  !Hilfsvariablen
		...
		if (mpi_rank == mpi_last) then
			temp(ubound(temp,1),:) = matrix(ubound(matrix,1),:)
		! Sonst empfangen
		else
			call MPI_IRECV(first, size(first), MPI_DOUBLE, (mpi_rank + 1), 2002, &
			& MPI_COMM_WORLD, request1, mpi_ierr)
			temp(ubound(temp,1),:) = first(:)
		end if
