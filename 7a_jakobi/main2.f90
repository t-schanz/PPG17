program poisson
	...
	integer, parameter :: edgeLength = 184	! Kantenlänge der gesamten Matrix
        ...
        integer, parameter :: outputSize = 24	! Kantenlänge der ausgegebenen Matrix
	...
	last = mpi_size - 1
        ...
        if (mpi_rank == master) then
                if (mod(edgeLength,(outputSize-1)) /= 0) then
                        write(*,*) "Die Variable outputSize muss so gewählt werden, dass bei der ", & 
                        &          "Rechnung [edgeLength / ( outputSize - 1 )] kein Rest entsteht"
                        stop
                endif
        endif
...
!	endif
	...
	!calculating:
        do i=1,loopSize
            ...
            call writeTempEdges(matrix,temp,mpi_rank,master,last)
            call sendrec(temp,mpi_rank, master,last,status(MPI_STATUS_SIZE))
